...
object Commitments {
        ...
        val htlcSigs = sortedHtlcTxs.collect {
          ...
          case tx: HtlcSuccessTx =>
            originChannels(tx.htlc.id) match {
              case _: Local =>
                keyManager.sign(tx, keyManager.htlcPoint(localParams.channelNumber), remoteNextPerCommitmentPoint)
              case r: Relayed =>
                keyManager.sign(tx, keyManager.htlcPoint(localParams.channelNumber), remoteNextPerCommitmentPoint, r.proof)
            }
        }
  ...
  def receiveCommit(commitments: Commitments, commit: CommitSig, keyManager: KeyManager)(implicit log: LoggingAdapter): (Commitments, RevokeAndAck) = {
    ...
    val htlcSigs = sortedHtlcTxs.collect {
      case tx: HtlcTimeoutTx => keyManager.sign(tx, keyManager.htlcPoint(localParams.channelNumber), localPerCommitmentPoint)
      case tx: HtlcSuccessTx => keyManager.sign(tx, keyManager.htlcPoint(localParams.channelNumber), localPerCommitmentPoint)
    }
